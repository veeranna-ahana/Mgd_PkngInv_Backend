...
inspectionProfileRouter.post(
	"/getOrderScheduleData",
	async (req, res, next) => {
		// Update operation to set QtyProduced in orderscheduledetails table
		try {
         ...
         AND o.SchDetailsId = t.SchDetailsId;`,
				(err, updateResult) => {
					if (err) {
						...
						// If update succeeds, proceed with fetching other data
						try {
							// Fetch header data
							misQueryMod(
								`SELECT
                    ...
                    magodmis.orderschedule.ScheduleId = ${req.body.scheduleID}`,
								(err, headerData) => {
									if (err) {
										...
										// Initialize InDraftPN variable
										let InDraftPN = 0;

										// Fetch order schedule details data
										try {
											misQueryMod(
												`SELECT * FROM magodmis.orderscheduledetails WHERE magodmis.orderscheduledetails.ScheduleId = ${req.body.scheduleID}`,
												(err, orderScheduleDetailsData) => {
													if (err) {
														...
														// Update SQL mode to handle GROUP BY
														try {
															misQueryMod(
																`SET @@sql_mode = REPLACE(@@sql_mode, 'ONLY_FULL_GROUP_BY', '')`,
																(err, updateGroupBy) => {
																	if (err) {
																		...
																		// Fetch draft DC inventory details
																		try {
																			misQueryMod(
																				`SELECT
                                            *,
                                         ...
                                         GROUP BY OrderSchDetailsID, DespStatus`,
																				(err, draftDCInvDetailsData) => {
																					if (err) {
																						...
																						// Add InDraftPN column to orderScheduleDetailsData
																						for (
																							let i = 0;
																							i <
																						...
																						// Match and update InDraftPN values
																						for (
																							let i = 0;
																							i <
																							orderScheduleDetailsData.length;
																						...
																						// Fetch draft DC inventory register data
																						try {
																							misQueryMod(
																								`SELECT
                                                    *,
                                                    ...
                                                    ScheduleId = ${req.body.scheduleID}`,
																								(err, invRegisterData) => {
																									if (err) {
																										...
																										// Fetch draft DC inventory details data
																										try {
																											misQueryMod(
																												`SELECT * FROM magodmis.draft_dc_inv_details WHERE ScheduleId = ${req.body.scheduleID}`,
																												(
